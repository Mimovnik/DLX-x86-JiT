...
void DLXJITx64::compileDLXInstruction(const DLXJITCodLine& line)
{
	if (line.textInstruction->opcode() == "ADD")
	{
		...
		if (destination > 0) //Zapisywanie do rejsetru R0 jest niedozwolone
		{
			...
			writeMovSXDMem32toReg64(RAX, getDLXRegisterOffsetOnStack(s1), RBP); //Odczyt pierwszego rejestru �r�d�owego DLX
			writeMovSXDMem32toReg64(RDX, getDLXRegisterOffsetOnStack(s2), RBP); //Odczyt drugiego rejestru �r�d�owego DLX
			writeAdd(RAX, RDX);
	...
	else if (line.textInstruction->opcode() == "LDW")
	{
		...
		if (destination > 0) //Zapisywanie do rejsetru R0 jest niedozwolone
		{
			...
			writeMovMem32toReg32(EAX, offset, x1, RDX, DATA_POINTER_REGISTER); //Odczyt danych z pami�ci
			writeBswap(EAX); //Zamiana endian�w
			//Niepotrzebne: writeMovSXDReg32toReg64(RAX, EAX); //Rozszerzanie liczby to 64 bit�w
			writeMovReg32toMem32(getDLXRegisterOffsetOnStack(destination), RBP, EAX); //Zapis wyniku
		}
	}
	else if (line.textInstruction->opcode() == "BRLE")
	{
		...
		if (dlxOffsetsInRawCode.size() > destDlxInstr)
			...
			jumpOffsetsToRepair.push_back({ instr, rawCode.size() }); //Skok w prz�d!
		writeJLE(offset);
	}
...
void DLXJITx64::compile()
{
	...
	writeAdd(RSP, 4 * numberOfDLXRegisters); //Dealokacja rejestr�w DLX ze stosu
	writeMovR64toR64(RSP, RBP);
	writePop(RBP);
	writeRet();

	for (auto& toRepair : this->jumpOffsetsToRepair) //Naprawianie skok�w w prz�d
	{
